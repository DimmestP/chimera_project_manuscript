...
```{r QuantSeq-sample-to-construct}
read.csv(here("./supplementary_data_chapter/data/QuantSeq_samplesheet.csv")) %>%
  dplyr::transmute(`Sample Name` = Sample_Name,  `Construct Name` = Construct_Name)  %>%
knitr::kable(format = "latex", caption = "\\textbf{QuantSeq sample name to construct table}.")  %>%
  kableExtra::kable_styling(latex_options = "hold_position") 
```

```{r 5PSeq-sample-to-construct}
read.csv(here("./supplementary_data_chapter/data/5PSeq_samplesheet.csv")) %>%
  dplyr::transmute(`Sample Name` = Sample_Name,  `Construct Name` = Construct_Name)  %>%
knitr::kable(format = "latex", caption = "\\textbf{5PSeq sample name to construct table}")  %>%
  kableExtra::kable_styling(latex_options = "hold_position") 
```

```{r rnaseq-QC-correlation, fig.cap="\\textbf{Sample correlation across both RNASeq experiments.} (\\textbf{A}) Correlation across samples for the QuantSeq assay. (\\textbf{B}) Correlation across samples for the 5PSeq assay.", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/sequencing_sample_correlation.png"))
```

```{r rnaseq-QC-QuantSeq-genomic-polyA, fig.cap="\\textbf{Poly(A) site usage for genomic PGK1, TSA1 and RPS3 teminators across QuantSeq samples.}", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/polya_QC_plot_QuantSeq.png"))
```

```{r rnaseq-QC-5PSeq-genomic-polyA, fig.cap="\\textbf{Poly(A) site usage for genomic PGK1, TSA1 and RPS3 teminators across 5PSeq samples.}", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/polya_QC_plot_5PSeq.png"))
```

```{r rnaseq-QC-genomic-vs-plasmid-polyA, fig.cap="\\textbf{Poly(A) site usage for genomic TSA1 and RPS3 teminators against plasmid constructs in QuantSeq and 5PSeq.}", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/plasmid_vs_genome_polyA_combined_plot.png"))
```

```{r rnaseq-QC-QuantSeq-vs-5PSeq, fig.cap="\\textbf{QuantSeq and 5PSeq log2 pseudocounts across all genes for samples with the same constructs. Samples are clustered by correlation.}", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/pRPS3_QuantSeq_5PSeq_correlation_comparison.png"))
```
