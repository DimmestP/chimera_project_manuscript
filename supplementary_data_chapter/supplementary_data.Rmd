...
```{r rnaseq-QC-correlation, fig.cap="\\textbf{High correlation in transcript counts between samples from the same RNA-Seq assay.} Correlaton between DESeq2 normalised log2 pseudocounts for all genes in each sample for QuantSeq and 5PSeq. (\\textbf{A}) From top to bottom on the y axis (left to right on the x-axis) the terminator contexts are pRPS3-tRPS3, pPGK1-tRPS3 and pTSA1-tTSA1.", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/sequencing_sample_correlation.png"))
```

```{r rnaseq-QC-QuantSeq-and-5PSeq-genomic-polyA, fig.cap="\\textbf{Poly(A) site usage for genomic PGK1, TSA1 and RPS3 teminators remains the same across samples for each RNA-Seq assay.}", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/polya_QC_plot_QuantSeq_and_5PSeq.png"))
```

```{r rnaseq-QC-genomic-vs-plasmid-polyA, fig.cap="\\textbf{Poly(A) site usage remains the same for genomic TSA1 and RPS3 teminators as for plasmid expressed WT constructs in QuantSeq and 5PSeq.}", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/plasmid_vs_genome_polyA_combined_plot.png"))
```

```{r rnaseq-QC-QuantSeq-vs-5PSeq, fig.cap="\\textbf{QuantSeq and 5PSeq genome wide transcript abundances are not correlated. }DESeq2 normalised log2 pseudocounts for QuantSeq and 5PSeq are plotted against one another.", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/pRPS3_QuantSeq_5PSeq_comparison.png"))
```

```{r relative-polya-counts, fig.cap="\\textbf{Construct poly(A) site usage across 5PSeq and QuantSeq.} Alternative to cumulative relative counts plots to compare changes in Poly(A) site usage across the constructs in both RNA-Seq assays.", fig.align='center',out.width="85%"}
