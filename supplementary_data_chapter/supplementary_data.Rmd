...
knitr::kable(tibble(CRE = c("Codon Usage", "Motifs", "3'UTR Length"), Individual = c("42.0%", "3.2%", "0.6%"), Drop = c("-40.5%", "-1.6%", "0.0%"), Cumulative = c("42.0%", "43.7%", "43.7%")), caption = "\\textbf{Variance explained by each predicate in the half life model}. Variance explained was explored in three different ways. The variance explained by a linear containing each CRE appearing individually.  The loss in variance explained when one CRE is removed from the full model. The total variance explained when each CRE is added to the linear model in sequence.", format = "latex")  %>%
  kableExtra::kable_styling(latex_options = "hold_position")
```


```{r protein-vs-RNA-plot-motifs, fig.cap="\\textbf{Motif contributions to protein abundance correlate with mRNA abundance}. (\\textbf{Left}) Fold changes in mRNA abundance vs fold changes in protein fluorescence for all terminator constructs in the pRPS3-tRPS3 and pTSA1-tTSA1 pairings. Transcript abundance is relative to the mod0 construct of each promoter-terminator pairing. (\\textbf{Right}) Fold changes in mRNA abundance vs fold changes in protein fluorescence for all terminator constructs in the pPIR1-tPIR1 pairing. Transcript abundance is relative to WT construct of the promoter-terminator pairing. In all figures, the mean and standard error calculated over 6 biological replicates is plotted for each construct.", fig.align='center'}
...
knitr::kable(format = "latex", caption="\\textbf{Selecting the HWNCAUUWY motif, TTTCATTTC}. Summary of the number of occurances and contributions to a linear model predecting half life for each possible version of the HWNCAUUWY motif.")  %>%
  kableExtra::kable_styling(latex_options = "hold_position")
```
...
read.csv(here("./supplementary_data_chapter/data/UGUAHMNUA_shortlisted_motif_table.csv")) %>%
  dplyr::transmute(Motif = term,  `Total Count` = total_count, Coef = estimate, `p.value`)  %>%
knitr::kable(format = "latex", caption = "\\textbf{Selecting the UGUAHMNUA motif, TGTACAAUA}. Summary of the number of occurances and contributions to a linear model predecting half life for each possible version of the UGUAHMNUA motif.")  %>%
  kableExtra::kable_styling(latex_options = "hold_position")
```
