---
title: "RT-qPCR of pRPS3-mCh-tRPS3mod experimental replicate 2 (n=2)"
...
knitr::opts_chunk$set(warning=FALSE,message=FALSE,echo=TRUE,cache=FALSE,
                      results="show",
                      fig.path="figures/analyse-qPCR-2020-02-04_pRPS3-tRPS3mod",
                      cache.path="cache/analyse-qPCR-2020-02-04_pRPS3-tRPS3mod")
...
Strainlevels <- c("pRPS3-tRPS3_WT","pRPS3-tRPS3_mod0","pRPS3-tRPS3_modA","pRPS3-tRPS3_modB","pRPS3-tRPS3_modC", "pRPS3-tRPS3_modD", "pRPS3-tRPS3_modE","POT1-ccdB")
...
Here, only one experimental replicate is loaded (n=2)
```{r load_plates,dependson="label_plates",results="show"}
# read my plates
plates <- read_tsv( "data/JA_20200204-pRPS3-tRPS3mod-n2-ct.txt",skip=1) %>%
...
Most -RT values are CT of ~30. -RT samples of pRPS3-mCh-tRPS3 mod0, especially biorep 3 seems low (CT of ~25). The pRPS3-mCh-tRPS3_modE biorep 2 CT values of all 4 probes are all higher than the other bioreps for all 4 probes.
...
-Previous experimental replicate showed mRNA abundance proportional to the number of incorporated motifs (modA-contains one decay motif, modD-contains two decay motifs, modE-contains three decay motifs) however here, we don't see much differernce between modA and modD

-Similar to the previous experimental replicate, there doesn't seem to be much difference between WT and mod0
-Similar to the previous experimental rerplicate, there isn't an increase in mRNA abundance in the 3'UTR with an incorporated putative stability motif
...
platecurve <- read_tsv("data/JA_20200204-pRPS3-tRPS3mod-n2.txt",
                       skip=2,
...
-RT seems to have a peak in all melt curves for all probes!
...
##Moving forward
-This experirmental replicate show similar results, thus validating the conclusions from the previous replicate
-The result from n=2 doesn't show the proportional mRRNA abundance in the linear combination of decay motifs

-Again, (Check with Sam and Edward)- an error appeared after knitting - "There were 50 or more warnings (use warnings() to see the first 50)"
