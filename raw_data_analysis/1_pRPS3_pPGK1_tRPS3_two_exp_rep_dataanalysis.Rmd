...
```{r unnorm_pRPS3,dependson="load_plates",fig.height=5,fig.width=10, echo=FALSE}
...
```{r plotamp_Rep1_P1,dependson="load_amp",fig.width=8,fig.height=6, echo=FALSE}
...
```{r plotmelt_Rep1,dependson="load_amp",fig.width=8,fig.height=6, echo=FALSE}
...
```{r export_normdata, echo=TRUE}
#exporting platesnorm dataframe
write.csv(platesnorm,"analysed_data/platesnorm.csv", row.names = FALSE)
```
...
One **Mean Value.norm of pRPS3-mCherry-tRPS3_mod0** is calculated by taking the median Value.norm among TechReps of each BioRep (of pRPS3-mCherry-tRPS3_mod0), then calculating **one** mean Value.norm among BioReps of pRPS3-mCherry-tRPS3_mod0. 

This value is used to then calculate the delta-delta Cq for all other strains. Delta delta Cq is calculated by subtracting the **Mean Value.norm of pRPS3-mCherry-tRPS3_mod0** from each Value.norm of all other strains. (NOTE - we still have one Value.norm for each TechRep and BioRep here). 
...
# Extracts the pRPS3 strains and deltaCq for mCherry (mCh-7)
platesnorm_pRPS3 <- platesnorm %>% filter(Promoter %in% "pRPS3", TargetID %in% "mCh-7")
  
# Extracting datapoints for pRPS3-mCherry-tRPS3_mod0 Samples. Calculates the median of each BioRep (collapses TechReps) then the mean Cq  of the mod0 strain (collapses BioReps) (-0.865)
mean_platesnorm_pRPS3mod0 <- platesnorm_pRPS3 %>% 
    select(c("Strain","Value.norm", "BioRep", "TechRep")) %>% 
    filter(Strain %in% c("pRPS3-mCherry-tRPS3_mod0")) %>%
    group_by(Strain, BioRep)%>%
    summarize(median_BioRep_pRPS3mod0 = median(Value.norm)) %>%
    ungroup()%>%
    group_by(Strain)%>%
    summarize(mean_pRPS3mod0 = mean(median_BioRep_pRPS3mod0))

# Calculates the delta delta Cq (subtracts mean(pRPS3mod0$Value.norm) from every Value.norm))
platesnorm_mod0_pRPS3 <- platesnorm_pRPS3 %>% 
    mutate(Value.norm.mod0 = Value.norm - mean_platesnorm_pRPS3mod0$mean_pRPS3mod0, remove=FALSE)
```

One **Mean Value.norm of pPGK1-mCherry-tRPS3_mod0** is calculated by taking the median Value.norm among TechReps of each BioRep (of pPGK1-mCherry-tRPS3_mod0), then calculating **one** mean Value.norm among BioReps of c-mCherry-tRPS3_mod0. 
...
**NOTE - Here median iis used to summarise/collapse TechReps and mean is used to summarise/collapse BioReps**
```{r delta-deltaCq_norm_PGK1, echo=TRUE}

# Extracts the pPGK1 strains
platesnorm_pPGK1 <- platesnorm %>% filter(Promoter %in% "pPGK1", TargetID %in% "mCh-7")

# Extracting datapoints for pPGK1-mCherry-tRPS3_mod0 Samples. Calculates the median of each BioRep (collapses TechReps) then the mean Cq  of the mod0 strain (collapses BioReps) (-2.235)
mean_platesnorm_pPGK1mod0 <- platesnorm_pPGK1 %>% 
    select(c("Strain","Value.norm", "BioRep", "TechRep"))%>% 
    filter(Strain %in% c("pPGK1-mCherry-tRPS3_mod0"))%>%
    group_by(Strain, BioRep)%>%
    summarize(median_BioRep_pPGK1mod0 = median(Value.norm)) %>%
    ungroup()%>%
    group_by(Strain)%>%
...
platesnorm_mod0_pPGK1 <- platesnorm_pPGK1 %>% 
    mutate(Value.norm.mod0 = Value.norm - mean_platesnorm_pPGK1mod0$mean_pPGK1mod0, remove=FALSE)

# to verify the calculation done 
platesnorm_mod0_pPGK1 %>% select(Strain, BioRep, TechRep, Value.norm.mod0)
```

### 2A) Exporting analysed delta Cq values data
```{r export_deltadeltaCq, echo=TRUE}
# Combining the calculated delta-delta Cq from the two dataframes
platesnorm_deltadeltaCq <- bind_rows(platesnorm_mod0_pRPS3, platesnorm_mod0_pPGK1)%>%
...
#exporting platesnorm_deltadeltaCq dataframe
write.csv(platesnorm_deltadeltaCq,"analysed_data/platesnorm_deltadeltaCq.csv", row.names = FALSE)
```
...
platesnorm_mod0_median <- bind_rows(platesnorm_mod0_pRPS3, platesnorm_mod0_pPGK1)%>%
...
# to verify the calculation done 
platesnorm_mod0_median %>% select(Strain, BioRep, Median_Cq, RNA_Abundance)
```

### 2A) Plotting the results in a scatter plot
These plots show the relative RNA abundance for each strain (relative to tRPS3-mod0). The results are split based on the promoter driving the expresssion of the mCherry CDS.

```{r norm_plot,fig.height=5,fig.width=9, echo=FALSE}

# Plotting the normalsied data (horizontal arrangement)
normalised_plot <- ggplot(data = platesnorm_mod0_median %>% 
...
normalised_plot + stat_summary(aes(RNA_Abundance,Ter_mod),
    fun="mean",colour="black",
    geom="crossbar",size=0.2, width=0.5) 
```

```{r norm_plot_1,fig.height=9,fig.width=5, echo=FALSE}

# Plotting the normalsied data (vertical arrangement)
normalised_plot_1 <- ggplot(data = platesnorm_mod0_median %>% 
    filter(!Strain %in% c("pPGK1-mCherry-tRPS3_WT","pRPS3-mCherry-tRPS3_WT")))+
    geom_point(aes(RNA_Abundance,Ter_mod,colour=Ter_mod, shape=BioRep)) +
...
normalised_plot_1 + stat_summary(aes(RNA_Abundance,Ter_mod),
    fun="mean",colour="black",
    geom="crossbar",size=0.2, width=0.5) 
```


## 2B) Delta-delta Cq and RNA abundance calculation (one mod0 mean per ExpRep per promoter)
Normalisation of delta Cq values against tRPS3-mod0 delta Cq values. Strains are first subdivided based on their promoter (either pPGK1 or pRPS3).   
...
# Calculates the mean Cq value of pRPS3-mCherry-tRPS3_mod0 Samples (-0.3833333	)
...
# Calculates the mean Cq value of pRPS3-mCherry-tRPS3_mod0 Samples (-1.346667	)
...
platesnorm_mod0_pRPS3_ExpRep1 <- platesnorm_pRPS3_ExpRep %>% filter(ExpRep==1)%>%
    mutate(Value.norm.mod0 = Value.norm - mean_platesnorm_pRPS3mod0_ExpRep1$mean_pRPS3mod0, remove=FALSE)
platesnorm_mod0_pRPS3_ExpRep2 <- platesnorm_pRPS3_ExpRep %>% filter(ExpRep==2)%>%
    mutate(Value.norm.mod0 = Value.norm - mean_platesnorm_pRPS3mod0_ExpRep2$mean_pRPS3mod0, remove=FALSE)

# to verify the calculation done 
platesnorm_mod0_pRPS3_ExpRep1 %>% select(Strain, BioRep, TechRep, Value.norm.mod0)
```
...
# Calculates the mean Cq value of pPGK1-mCherry-tRPS3_mod0 Samples (-2.213333)
...
# Calculates the mean Cq value of pPGK1-mCherry-tRPS3_mod0 Samples (-2.256667)
...
platesnorm_mod0_pPGK1_ExpRep1 <- platesnorm_pPGK1_ExpRep %>% filter(ExpRep==1)%>%
    mutate(Value.norm.mod0 = Value.norm - mean_platesnorm_pPGK1mod0_ExpRep1$mean_pPGK1mod0, remove=FALSE)
platesnorm_mod0_pPGK1_ExpRep2 <- platesnorm_pPGK1_ExpRep %>% filter(ExpRep==2)%>%
    mutate(Value.norm.mod0 = Value.norm - mean_platesnorm_pPGK1mod0_ExpRep2$mean_pPGK1mod0, remove=FALSE)

# to verify the calculation done 
platesnorm_mod0_pPGK1_ExpRep1 %>% select(Strain, BioRep, TechRep, Value.norm.mod0)
```
...
### 2B) Exporting analysed data
```{r}
#exporting platesnorm_mod0_median_ExpRep dataframe
write.csv(platesnorm_mod0_median_ExpRep,"analysed_data/platesnorm_mod0_median_ExpRep.csv", row.names = FALSE)
```

### 2B) Plotting the results in a scatter plot

```{r norm_plot_ExpRep,fig.height=5,fig.width=9, echo=FALSE}
...
```{r norm_plot1_ExpRep,fig.height=9,fig.width=5, echo=FALSE}
