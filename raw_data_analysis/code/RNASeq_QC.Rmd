...
pRPS3_QuantSeq_5PSeq_correlation_comparison <- pheatmap(cbind(subset(assay(log_2_plus_one_5PSeq), 
                      select = rownames(heatmap_label_5PSeq_match_QuantSeq)),
               subset(assay(log_2_plus_one_QuantSeq), 
                      select = rownames(heatmap_label_QuantSeq_pRPS3))),
         cluster_rows=FALSE,
         show_rownames = FALSE,
         clustering_distance_cols="correlation",
         annotation_col=rbind(cbind(heatmap_label_QuantSeq_pRPS3,
                                    "assay" = "5PSeq"),
                              cbind(heatmap_label_5PSeq_match_QuantSeq,
                                    "assay" = "QuantSeq")))
...
factor_levels_5PSeq <- c("1dt", "2dt", "3dt", "4dt", "5dt", "6dt",
                         "7dt", "8dt", "9dt", "10dt", "11dt", "12dt",
                         "13dt", "14dt", "15dt", "16dt")

sample_correlation_quantseq<- ggplot(as_tibble(cor(assay(log_2_plus_one_QuantSeq)), 
                 rownames = "sample_x") %>% 
         pivot_longer(-sample_x, 
                      names_to = "sample_y", 
                      values_to = "cor") %>% 
         mutate(sample_x = factor(sample_x, factor_levels_QuantSeq), 
                sample_y = factor(sample_y, factor_levels_QuantSeq))) + 
...
sample_correlation_5Pseq <- ggplot(as_tibble(cor(assay(log_2_plus_one_5PSeq)), 
                 rownames = "sample_x") %>% 
         pivot_longer(-sample_x, 
                      names_to = "sample_y", 
                      values_to = "cor") %>% 
         mutate(sample_x = factor(sample_x, factor_levels_5PSeq), 
                sample_y = factor(sample_y, factor_levels_5PSeq))) + 
    geom_tile(aes(x= sample_x, y = sample_y, fill = cor)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
  coord_equal() +
  labs(x= "", y = "")
...
ggsave2(here("supplementary_data_chapter/figures/pRPS3_QuantSeq_5PSeq_correlation_comparison.png"),
        pRPS3_QuantSeq_5PSeq_correlation_comparison,
        width = fig_width_2column,
        height = 210,
        units = "mm",
        dpi = fig_dpi,
