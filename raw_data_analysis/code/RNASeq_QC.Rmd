...
heatmap_metadata_QuantSeq <- as.data.frame(colData(DESeq_dataset_QuantSeq)[,c("promoter","mod_label")])

heatmap_metadata_5PSeq <- as.data.frame(colData(DESeq_dataset_5PSeq)[,c("promoter","mod_label")])

log_2_plus_one_QuantSeq <- normTransform(DESeq_dataset_QuantSeq)

log_2_plus_one_5PSeq <- normTransform(DESeq_dataset_5PSeq)

pheatmap(assay(log_2_plus_one_QuantSeq),
         cluster_rows=FALSE,
         show_rownames = FALSE,
         clustering_distance_cols="correlation",
         annotation_col=heatmap_metadata_QuantSeq)

pheatmap(assay(log_2_plus_one_5PSeq),
         cluster_rows=FALSE,
         show_rownames = FALSE,
         clustering_distance_cols="correlation",
         annotation_col=heatmap_metadata_5PSeq)
...
ggplot(as_tibble(cor(assay(log_2_plus_one_QuantSeq)), 
                 rownames = "sample_x") %>% 
...
ggplot(as_tibble(cor(assay(log_2_plus_one_5PSeq)), 
                 rownames = "sample_x") %>% 
         pivot_longer(-sample_x, 
                      names_to = "sample_y", 
                      values_to = "cor") %>% 
