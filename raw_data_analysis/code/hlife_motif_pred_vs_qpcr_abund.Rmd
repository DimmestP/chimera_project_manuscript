...
r_squared_data_TSA1_RPS3 <- combined_pred_vs_actual_abund_TSA1_RPS3 %>% 
...
r_squared_data_PIR1 <- pPIR1_pPGK1_pSRO9_tPIR1_norm_mean %>% 
  group_by(construct, terminator, promoter) %>% 
  summarise(delta_delta_cq = mean(delta_delta_cq), .groups = "drop") %>%
...
pred_vs_abundance_TSA1_RPS3 <- ggplot(combined_pred_vs_actual_plot_data_TSA1_RPS3) + 
  ...
  geom_text(aes(label = r_squared_label), data = r_squared_data_TSA1_RPS3, x = -1.2, y= 1, parse = TRUE, size = 4) +
...
pred_vs_abundance_PIR1 <- ggplot(pred_vs_actual_plot_data_PIR1) + 
  ...
  geom_text(aes(label = r_squared_label), data = r_squared_data_PIR1, y = -0.7, x= 0, parse = TRUE, size = 4) +
  theme(panel.grid.minor = element_blank(), legend.position = "right") +
  scale_color_brewer(palette = "Paired")
