...
pRPS3_pPGK1_pSRO9_tRPS3_norm_mean <- read_csv(here("./raw_data_analysis/data/norm_qpcr/pRPS3_pPGK1_pSRO9_tRPS3_deltacq_platesnorm_summarise.csv")) %>%
  ...
  group_by(promoter) %>%
  mutate(mod0_delta_cq = filter(., construct == "mod0") %>% pull(deltacq) %>% mean()) %>%
...
construct_dictionary <- tibble( pro = c("PGK1", "Native", "PGK1", "Native", "SRO9"), ter = c("TSA1", "TSA1", "RPS3", "RPS3", "RPS3"), ter_pro = c("pPGK1_tTSA1", "pTSA1_tTSA1", "pPGK1_tRPS3", "pRPS3_tRPS3", "pSRO9_tRPS3"))

r_squared_data <- combined_pred_vs_actual_abund %>% 
  group_by(construct, ter_pro) %>% 
...
combined_pred_vs_actual_plot_data <- combined_pred_vs_actual_abund %>%
  ...
  group_by(construct, mean_delta_delta_cq, ter, pro) %>% 
