...
pred_vs_abundance_RPS3 <- ggplot(combined_pred_vs_actual_plot_data_TSA1_RPS3 %>% filter(terminator == "tRPS3")) + 
  pred_vs_abundance_figure_options +
  coord_fixed(ratio = 1 , xlim = c(-2.5, 1.5) ,ylim = c(-2.5, 1.5)) +
  geom_text(aes(label = r_cor_label), data = r_cor_data_TSA1_RPS3 %>% filter(terminator == "tRPS3"), x = -1.2, y= 1, parse = TRUE, size = 4)

pred_vs_abundance_TSA1 <- ggplot(combined_pred_vs_actual_plot_data_TSA1_RPS3 %>% filter(terminator == "tTSA1")) + 
  pred_vs_abundance_figure_options +
  coord_fixed(ratio = 1 , xlim = c(-2.5, 1.5) ,ylim = c(-2.5, 1.5)) +
  geom_text(aes(label = r_cor_label), data = r_cor_data_TSA1_RPS3 %>% filter(terminator == "tTSA1"), x = -1.2, y= 1, parse = TRUE, size = 4) +
  theme(strip.text.x = element_blank()) +
  labs(y = TeX("Experimental log_2 Fold Change"))
...
pred_vs_abundance_PIR1 <- ggplot(pred_vs_actual_plot_data_PIR1) + 
  ...
  theme(strip.text.x = element_blank()) +
  labs(x=TeX("Predicted log_2 Fold Change"))

wrap_plots(pred_vs_abundance_RPS3,
           pred_vs_abundance_TSA1,
