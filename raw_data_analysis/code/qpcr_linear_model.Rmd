...
pSRO9_tRPS3_norm_mean <- read_csv(here("./raw_data_analysis/data/norm_qpcr/pSRO9_tRPS3_n1_n2_norm_mean.csv")) %>%
  group_by(construct, bio_rep, exp_rep, terminator, promoter) %>%
  summarise(delta_cq = median(delta_cq), .groups = "drop") %>%
  inner_join(construct_to_motif_dictionary) %>%
  spread(key= motif, value = count) %>%
...
collated_norm_mean_data <- pPGK1_tRPS3_norm_mean %>%
  ...
  bind_rows(pSRO9_tRPS3_norm_mean) %>%
  inner_join(construct_to_motif_dictionary) %>%
  spread(key= motif, value = count)
...
pSRO9_tRPS3_norm_mean_lm <- lm(delta_cq ~ exp_rep + ATATTC + GTATACCTA + TGTACAATA + TTTCATTTC + TTTCATTTC:TGTACAATA + native, pSRO9_tRPS3_norm_mean)
...
model_r_squares <- tibble(
  ter_pro = c("pTSA1_tTSA1", "pPGK1_tTSA1", "pRPS3_tRPS3", "pPGK1_tRPS3", "pSRO9_tRPS3"),
...
construct_dictionary <- model_coefficients %>% 
  distinct(ter_pro) %>%
  mutate(promoter_type = c("PGK1", "Native", "PGK1", "Native", "SRO9"))
...
individual_linear_model_coefficients_points <- ggplot(model_coefficients %>% 
         ...
         arrange(term,promoter),
       aes(x=factor(promoter_type, levels = c("Native","PGK1","SRO9")), y=-estimate, colour = terminator, alpha = (p.value < 0.05)),
