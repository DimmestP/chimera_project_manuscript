...
norm_allplot_high <- ggplot(data=norm_pro_mTurq_ter_all %>% filter(!Promoter %in% c('pCLN2', 'pSRO9')), aes(x=Terminator,y=norm_mTurq_per_OD_at_max_gr)) +
  geom_point(aes(color=Terminator),
...
norm_allplot_high + stat_summary(aes(Terminator,norm_mTurq_per_OD_at_max_gr),
    fun="mean",colour="black",
    geom="crossbar",size=0.2, width=0.5) 
```
...
norm_allplot_low <- ggplot(data=norm_pro_mTurq_ter_all %>% filter(!Promoter %in% c('pHSP26', 'pPGK1', 'pRPS3', 'pRPS13')), aes(x=Terminator,y=norm_mTurq_per_OD_at_max_gr)) +
  geom_point(aes(color=Terminator),
               position=position_dodge(width = 0.5),size=2.3, alpha=0.8) +
...
norm_allplot_low + stat_summary(aes(Terminator,norm_mTurq_per_OD_at_max_gr),
    fun="mean",colour="black",
    geom="crossbar",size=0.2, width=0.5) +
