...
class platereader:
    ...
    p.correctOD()
    p.plot(y= 'OD')
    p.plot(y= 'OD', hue= 'strain', conditionincludes= ['Gal', 'Glu'], strainexcludes= 'HXT7')
    p.correctmedia()
    p.correctauto()
    ...
    def __init__(self, dnames= False, anames= False,  wdir= '', platereadertype= 'Tecan', dsheetnumbers= False,
                 asheetnumbers= False, ODfname= 'ODcorrection_Glucose_Haploid.txt', info= True, ls= True):
        '''
        ...
        ls: if True (default), display contents of working directory
        '''
        self.version= '0.19'
    ...
    def load(self, dnames, anames= False, platereadertype= 'Tecan', dsheetnumbers= False, asheetnumbers= False,
        ...
        for i, dname in enumerate(dnames):
            ...
            self.initialiseprogress(experiment)
            ...
            for x in np.arange(1,13):
                for y in 'ABCDEFGH':
                    ...
                    if isinstance(anno[x][y], str):
                        ...
                        rcontents[well]= [c.strip(), s.strip().replace('null', 'Null')]
    ...
    #####
    def plot(self, x= 'time', y= 'OD', by= False, hue= None, style= None, kind= 'line', size= None,
             col= None, row= None, height= 5, aspect= 1, ymin= False,
             onefig= True, title= False, plate= False, nonull= False, messages= False,
        ...
        by: variable to highlight – either 'well', 'experiment', 'condition', or 'strain' – using multiple figures
        hue: (Seaborn) variables that determines line colours
        ...
        if plate:
            ...
            if by:
                ...
                if by == 'well':
                    ...
                    if not style and len(cons) < snsmax:
                        style= 'condition'
                    elif not size:
                        size= 'condition'
                    if not style and len(exps) < snsmax: style= 'experiment'
                    ...
                    for e in exps:
                        for s in strs:
                            ...
                            if df.empty:
                                ...
                                lhue, lsize= self.checkforseaborn(df, hue, size)
                                sfig= sns.relplot(x, y, data= df, hue= lhue, kind= kind, ci= 'sd', style= style, size= lsize, **kwargs)
                                if title:
                                    sfig.fig.suptitle(title)
                                else:
                    ...
                    if not style and len(strs) < snsmax:
                        style= 'strain'
                    elif not size:
                        size= 'strain'
                    if not style and len(exps) < snsmax: style= 'experiment'
                ...
                if df.empty:
                    ...
                    if hue != 'experiment':
                        ...
                        if not size: size= 'experiment'
                    lhue, lsize= self.checkforseaborn(df, hue, size)
                    sfig= sns.relplot(x, y, data= df, hue= lhue, kind= kind, style= style, ci= 'sd', size= lsize,
                                      col= col, row= row, aspect= aspect, height= height, **kwargs)
    ...
    def checkforseaborn(self, df, hue, size):
        # prevent seaborn crashing when called with hue or size of length one
        if hue and df[hue].unique().size == 1: hue= None
        if size and df[size].unique().size == 1: size= None
        return hue, size
    ...
    #####
    def exportdf(self, commonname= False, type= 'json'):
        '''
        Exports the dataframes as either json (default) or tab-delimited csv files.
        ...
        type: type of file for export (either 'json' or 'csv' or 'tsv')
        '''
        self.logmethod(self.logger)
        if commonname:
        ...
        f.close()
        if self.wdir: print('Exported to', self.wdir)
    ...
    def importdf(self, commonnames, info= True, sep= '\t'):
        ...
        for commonname in commonnames:
            ...
            for df in ['r', 's', 'sc']:
                try:
                    ...
                    try:
                        # csv files
                        exec("impdf= pd.read_csv(commonname + '_' + df + '.csv', sep= ',')")
                        print('Imported', commonname + '_' + df +'.csv')
                    except FileNotFoundError:
                        try:
                            # tsv files
                            exec("impdf= pd.read_csv(commonname + '_' + df + '.tsv', sep= '\t')")
                            print('Imported', commonname + '_' + df +'.tsv')
                        except FileNotFoundError:
                            print('No file called', commonname + '_' + df + '.json or .csv or .tsv found')
                            return
        ...
        self.allconditions.update({e : list(pd.unique(self.s.loc[self.s['experiment'] == e]['condition']))
                                   for e in self.allexperiments})
        self.allstrains.update({e : list(pd.unique(self.s.loc[self.s['experiment'] == e]['strain']))
                                for e in self.allexperiments})
        # find datatypes with mean in self.s
        dtypdict= {e : list(self.s.query('experiment == @e').columns[self.s.query('experiment == @e').columns.str.contains('mean')]) for e in self.allexperiments}
        self.datatypes.update({e : [dt.split(' mean')[0] for dt in dtypdict[e]] for e in dtypdict})
        # initialise progress
        for e in self.allexperiments: self.initialiseprogress(e)
