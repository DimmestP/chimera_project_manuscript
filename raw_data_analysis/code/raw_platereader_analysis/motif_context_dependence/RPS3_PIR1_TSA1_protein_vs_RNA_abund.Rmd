...
RPS3_TSA1_protein <-
  bind_rows(RPS3_protein, TSA1_protein) %>%
...
```{r calc_protein_rna_correlation}
r_cor_data_RPS3_TSA1 <- 
  RPS3_TSA1_protein_and_RNA_data %>% 
  group_by(promoter) %>% 
  summarise(cor = round(cor(relative_abundance_mrna,relative_abundance_protein),2)) %>%
...
RPS3_TSA1_protein_and_RNA_plot <- 
  ggplot(RPS3_TSA1_protein_and_RNA_data) + 
  geom_point(aes(y = relative_abundance_protein, 
                 x = relative_abundance_mrna, 
                 colour = label), 
             alpha = 0.5) +
...
PIR1_protein_and_RNA_plot <- 
  ggplot(PIR1_protein_and_RNA_data) + 
