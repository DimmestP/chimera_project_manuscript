...
strainlevels_3 <- c("pSRO9-mCherry-tSRO9_WT-200bp", "pSRO9-mCherry-tSRO9_WT-543bp")
...
plateplan_A <- bind_rows(plateplan1, plateplan2, plateplan3)%>% 
    ...
    separate(strain, remove = FALSE,sep="-",into=c("Promoter","mCherry","Terminator","Length"))%>%
      mutate(UTR3 = factor(Length,levels = c("543bp","200bp","86bp","59bp")),remove = FALSE)%>%
...
platesnorm_summarise <- platesnorm%>%
    ...
    mutate(strain = factor(strain,levels = c("pRPS3-mCherry-tRPS3_WT-59bp", 
                                             "pRPS3-mCherry-tRPS3_WT-86bp", "pRPS3-mCherry-tRPS3_WT-200bp",
                                             "pSRO9-mCherry-tSRO9_WT-200bp", "pSRO9-mCherry-tSRO9_WT-543bp")),
...
# Calculates the mean cq value of pSRO9-mCherry-tSRO9_WT-543bp strain (4.111667)
mean_platesnorm_S543bp_ExpRep1 <- platesnorm_pSRO9_ExpRep %>% filter(ExpRep==1)%>%
    select(c("strain","delta_cq", "BioRep", "tech_rep")) %>% 
    filter(strain %in% c("pSRO9-mCherry-tSRO9_WT-543bp")) %>%
    group_by(strain, BioRep)%>%
    summarize(median_BioRep_S543bp = median(delta_cq)) %>%
    ungroup()%>%
    group_by(strain)%>%
    summarize(mean_S543bp = mean(median_BioRep_S543bp))

# Calculates the mean cq value of pSRO9-mCherry-tSRO9_WT-543bp Samples (4.035)
mean_platesnorm_S543bp_ExpRep2 <- platesnorm_pSRO9_ExpRep %>% 
                                          filter(ExpRep==2)%>%
    select(c("strain","delta_cq", "BioRep", "tech_rep")) %>% 
    filter(strain %in% c("pSRO9-mCherry-tSRO9_WT-543bp")) %>%
    group_by(strain, BioRep)%>%
...
# Calculates the delta delta cq (subtracts mean(mean_tSRO9543bp$delta_cq) from every delta_cq))
platesnorm_by_S543bp_pSRO9_ExpRep1 <- platesnorm_pSRO9_ExpRep %>% filter(ExpRep==1)%>%
    mutate(delta_deltacq = delta_cq - mean_platesnorm_S543bp_ExpRep1$mean_S543bp, remove=FALSE)
platesnorm_by_S543bp_pSRO9_ExpRep2 <- platesnorm_pSRO9_ExpRep %>% filter(ExpRep==2)%>%
    mutate(delta_deltacq = delta_cq - mean_platesnorm_S543bp_ExpRep2$mean_S543bp, remove=FALSE)
```
...
platesnorm_S543bp_pSRO9_median <- bind_rows(platesnorm_by_S543bp_pSRO9_ExpRep1,platesnorm_by_S543bp_pSRO9_ExpRep2)%>%
                    group_by(strain, target_id, BioRep, ExpRep)%>%
...
platesnorm_all_median <- bind_rows(platesnorm_S543bp_pSRO9_median,platesnorm_R200bp_pRPS3_median)
...
normalised_plot_SRO9 <- ggplot(data = platesnorm_S543bp_pSRO9_median) +
