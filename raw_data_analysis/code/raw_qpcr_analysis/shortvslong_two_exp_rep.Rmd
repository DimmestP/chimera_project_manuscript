...
# read my plates
plates_1 <- read_tsv(here("raw_data_analysis/data/raw_qpcr/JA_20200830-shortvslong_n1-ct.txt"),skip=1) %>%
    mutate(Well=Pos,Cq=Cp,ExpRep = 1,ExpRep=factor(ExpRep)) %>%
    left_join(plateplan_A)

plates_2 <- read_tsv(here("raw_data_analysis/data/raw_qpcr/JA_20200831-shortvslong_n2-ct.txt"),skip=1) %>%
...
platecurve_A <- read_tsv(here("raw_data_analysis/data/raw_qpcr/JA_20200830-shortvslong_n1.txt"),skip=2,
...
platecurve_B <- read_tsv(here("raw_data_analysis/data/raw_qpcr/JA_20200831-shortvslong_n2.txt"),skip=2,
                        col_names=c("Well","SID","Program","Segment","Cycle","Time","Temperature","Fluor")) %>%
...
normalised_Exp_pRPS3 <- ggplot(data = platesnorm_summarise %>% filter(Promoter %in% 'pRPS3'))+
    geom_point(aes(rel_abund_deltaCq,TargetID,colour=TargetID, shape=BioRep)) +
...
normalised_Exp_pRPS3 + stat_summary(aes(rel_abund_deltaCq,TargetID),
...
platesnorm_by_S500bp_pSRO9_ExpRep1 <- platesnorm_pSRO9_ExpRep %>% filter(ExpRep==1)%>%
    mutate(delta_deltaCq = Value.norm - mean_platesnorm_S500bp_ExpRep1$mean_S500bp, remove=FALSE)
platesnorm_by_S500bp_pSRO9_ExpRep2 <- platesnorm_pSRO9_ExpRep %>% filter(ExpRep==2)%>%
    mutate(delta_deltaCq = Value.norm - mean_platesnorm_S500bp_ExpRep2$mean_S500bp, remove=FALSE)
```
...
platesnorm_by_R200bp_pRPS3_ExpRep1 <- platesnorm_pRPS3_ExpRep %>% filter(ExpRep==1)%>%
    mutate(delta_deltaCq = Value.norm - mean_platesnorm_R200bp_ExpRep1$mean_R200bp, remove=FALSE)
platesnorm_by_R200bp_pRPS3_ExpRep2 <- platesnorm_pRPS3_ExpRep %>% filter(ExpRep==2)%>%
    mutate(delta_deltaCq = Value.norm - mean_platesnorm_R200bp_ExpRep2$mean_R200bp, remove=FALSE)
```
...
```{r plot_delta_deltaCq_SRO9,fig.height=2.5,fig.width=6.5, echo=FALSE}

normalised_plot_SRO9 <- ggplot(data = platesnorm_S500bp_pSRO9_median)+
    geom_point(aes(rel_abund_delta_deltaCq,Strain,colour=Ter_length, shape=BioRep)) +
...
normalised_plot_SRO9 + stat_summary(aes(rel_abund_delta_deltaCq,Strain),
    fun="mean",colour="black",
    geom="crossbar",size=0.2, width=0.5) 
```
...
normalised_plot_RPS3 + stat_summary(aes(rel_abund_delta_deltaCq,Strain),
    fun="mean",colour="black",
    geom="crossbar",size=0.2, width=0.5) 
```