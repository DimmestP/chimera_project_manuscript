...
# read my plates
plates_1 <- read_lightcycler_1colour_cq(here("raw_data_analysis/data/raw_qpcr/JA_20200830-shortvslong_n1-ct.txt")) %>%
  left_join(plateplan_A) %>%
  mutate(ExpRep = 1,ExpRep=factor(ExpRep))


plates_2 <- read_lightcycler_1colour_cq(here("raw_data_analysis/data/raw_qpcr/JA_20200831-shortvslong_n2-ct.txt")) %>%
...
platecurve_A <- read_lightcycler_1colour_raw(here("raw_data_analysis/data/raw_qpcr/JA_20200830-shortvslong_n1.txt")) %>%
  debaseline() %>%
  left_join(plateplan_A) %>%
  mutate(ExpRep = 1,ExpRep=factor(ExpRep))

platecurve_B <- read_lightcycler_1colour_raw(here("raw_data_analysis/data/raw_qpcr/JA_20200831-shortvslong_n2.txt")) %>%
  debaseline() %>%
  left_join(plateplan_B) %>%
  mutate(ExpRep = 2,ExpRep=factor(ExpRep))
...
ggplot(platesnorm, aes(Ter_length,delta_cq))+
...
ggplot(platesnorm %>% filter(Promoter %in% c("pRPS3")), aes(Ter_length,delta_cq))+
...
ggplot(platesnorm %>% filter(Promoter %in% c("pSRO9")), aes(Ter_length,delta_cq))+
  geom_point(aes(color=BioRep),position=position_dodge(width = 0.8),size=1.5, alpha=0.7)+
  scale_colour_hue(h = c(90, 360)+20,l=60,c=60)+
  ylim(-6,6)+
  labs(y="delta cq", x="3'UTR-terminators")+
...
ggplot(platesnorm %>% filter(target_id %in% 'mCh-7'), aes(Ter_length,delta_cq))+
...
normalised_all <- ggplot(data = platesnorm_summarise %>% filter(target_id %in% 'mCh-7'))+
    geom_point(aes(Ter_length,rel_abund_deltacq,colour=strain, shape=BioRep)) +
...
normalised_all + stat_summary(aes(Ter_length,rel_abund_deltacq),
    fun="mean",colour="black",
    geom="crossbar",size=0.2, width=0.5) 
```
...
normalised_plot_SRO9 <- ggplot(data = platesnorm_S500bp_pSRO9_median)+
    geom_point(aes(Ter_length,rel_abund_delta_deltacq,colour=Ter_length, shape=BioRep)) +
    scale_y_log2nice(name="Fold change in RNA abundance relative to \n pSRO9-mCh-tSRO9_500bp (log2 scale)",scilabels=FALSE) +
      labs(x="") +
    scale_shape_manual(values=c(19, 17, 15, 10, 7, 14)) +
    scale_colour_manual(values=c("#CC6666", "black")) +
    guides(colour=FALSE) +
    theme(axis.text.x=element_text(angle=0,vjust=0.5, colour=c("#CC6666", "black")),
          axis.title.x=element_text(vjust=-2),
          legend.position="right")
  #facet_wrap(~Pro_mCh,ncol = 2)

combined_SRO9_graph <- normalised_plot_SRO9 + stat_summary(aes(Ter_length,rel_abund_delta_deltacq),
    fun="mean",colour="black",
    geom="crossbar",size=0.2, width=0.5) 
```
...
normalised_plot_RPS3 <- ggplot(data = platesnorm_R200bp_pRPS3_median)+
    geom_point(aes(Ter_length,rel_abund_delta_deltacq, colour=Ter_length, shape=BioRep)) +
    scale_y_log2nice(name="Fold change in RNA abundance relative to \n pRPS3-mCh-tRPS3_200bp (log2 scale)",scilabels=FALSE) +
      labs(x="") +
    scale_shape_manual(values=c(19, 17, 15, 10, 7, 14)) +
    scale_colour_manual(values=c("#288a2e","#a84a9a","#6f3ba1")) +
    guides(colour=FALSE) +
    theme(axis.text.x=element_text(angle=0,vjust=0.5,colour=c("#288a2e","#a84a9a", "#6f3ba1")),
          axis.title.x=element_text(vjust=-2),
          legend.position="right")

combined_RPS3_graph <- normalised_plot_RPS3 + stat_summary(aes(Ter_length, rel_abund_delta_deltacq),
    fun="mean",colour="black",
    geom="crossbar",size=0.2, width=0.5) 

ggsave( here("./results_chapter/figures/RPS3_SRO9_short_vs_long.png"),plot_grid(combined_RPS3_graph, combined_SRO9_graph, ncol=1), height = 10, width = 7)
```