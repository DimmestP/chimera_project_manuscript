...
### Amplification Curves of POT1-ccdB strains (all bioreps) against all 4 Targetids (mCh-7, PGK1-ORF, RPS3-ORF, URA3-ORF) (pTSA1)
...
ggplot(data=platesamp %>% filter(strain==c("POT1-ccdB")),
    ...
    expand_limits(y=0) + 
    labs(title="Amplification curves of POT1-ccdB strains (BioRep 1-6) in pTSA1 experiments") + panel_border()

ggplot(data=platesamp %>% filter(strain==c("pTSA1-mCherry-tTSA1_WT")),
       aes(x=cycle,y=fluor_signal,linetype=prep_type,group=well, color=tech_rep)) + 
    facet_grid(bio_rep~target_id) + 
    geom_line() +
    scale_linetype_manual(values=c("-RT"="dashed","+RT"="solid")) +
    expand_limits(y=0) + 
    labs(title="Amplification curves of pTSA1-mCherry-tTSA1_WT strains (BioRep 1-6) in pTSA1 experiments") + panel_border()
```



### Melt Curves of POT1-ccdB and pTSA1-mCherry-tTSA1_WT strains (all bioreps) against all 4 Targetids (mCh-7, PGK1-ORF, RPS3-ORF, URA3-ORF) (pTSA1)
...
ggplot(data=platesmelt %>% filter(strain==c("POT1-ccdB")),
    ...
    labs(title="Melt curves of POT1-ccdB strains (BioRep 1-6) in pTSA1 experiments") + panel_border()

ggplot(data=platesmelt %>% filter(strain==c("pTSA1-mCherry-tTSA1_WT")),
       aes(x=temperature,y=dRdT,group=well)) + 
    facet_grid(bio_rep~target_id) + 
    geom_line(aes(color=exp_rep, linetype=prep_type)) +
    scale_linetype_manual(values=c("-RT"="dashed","+RT"="solid")) +
    scale_x_continuous(breaks=seq(60,100,10),minor_breaks=seq(60,100,5)) + 
    labs(title="Melt curves of pTSA1-mCherry-tTSA1_WT strains (BioRep 1-6) in pTSA1 experiments") + panel_border()
...
One **Mean Value.norm of pTSA1-mCherry-tTSA1_mod0** is calculated by taking the median Value.norm among TechReps of each BioRep (of pTSA1-mCherry-tTSA1_mod0), then calculating **one** mean Value.norm among BioReps of pTSA1-mCherry-tTSA1_mod0. 

This value is used to then calculate the delta-delta Cq for all other strains. Delta delta Cq is calculated by subtracting the **Mean Value.norm of pTSA1-mCherry-tTSA1_mod0** from each Value.norm of all other strains. (NOTE - we still have one Value.norm for each TechRep and BioRep here). 

**NOTE - Here median is used to summarise/collapse TechReps and mean is used to summarise/collapse BioReps**
```{r delta-deltaCq_norm_TSA1, echo=TRUE}

# Extracts the pTSA1 strains and deltaCq for mCherry (mCh-7)
platesnorm_pTSA1 <- platesnorm %>% filter(promoter %in% "pTSA1", target_id %in% "mCh-7")

# Save normalised TSA1 constructs
write_csv(platesnorm_pTSA1, here("raw_data_analysis/data/norm_qpcr/pTSA1_tTSA1_n1_n2_norm.csv"))
  
# Extracting datapoints for pTSA1-mCherry-tTSA1_mod0 Samples. Calculates the median of each BioRep (collapses TechReps) then the mean Cq  of the mod0 strain (collapses BioReps) (-0.865)
mean_platesnorm_pTSA1mod0 <- platesnorm_pTSA1 %>% 
    select(c("strain","delta_cq", "bio_rep", "tech_rep")) %>% 
    filter(strain %in% c("pTSA1-mCherry-tTSA1_mod0")) %>%
    group_by(strain, bio_rep)%>%
    summarize(median_BioRep_pTSA1mod0 = median(delta_cq)) %>%
    ungroup()%>%
    group_by(strain)%>%
    summarize(mean_pTSA1mod0 = mean(median_BioRep_pTSA1mod0))

# Calculates the delta delta Cq (subtracts mean(pTSA1mod0$Value.norm) from every Value.norm))
platesnorm_mod0_pTSA1 <- platesnorm_pTSA1 %>% 
    mutate(delta_delta_cq = delta_cq - mean_platesnorm_pTSA1mod0$mean_pTSA1mod0, remove=FALSE)
```
...
platesnorm_mod0_median <- platesnorm_mod0_pTSA1 %>%
          ...
          # Below is to create columns for strain information using details from the 'Strain' column
                    ...
                    unite(pro_mCh,promoter,mCherry,sep="-", remove=FALSE)%>%
                    unite(ter_mod,terminator,construct,sep="_", remove=FALSE) %>%
                    mutate(tro_mCh = factor(pro_mCh,levels = c("pTSA1-mCherry","pPGK1-mCherry"))) %>% 
...
normalised_plot <- ggplot(data = platesnorm_mod0_median %>% 
    filter(!strain %in% c("pPGK1-mCherry-tTSA1_WT","pTSA1-mCherry-tTSA1_WT")))+
