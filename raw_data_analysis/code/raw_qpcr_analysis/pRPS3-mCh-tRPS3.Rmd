---
title: "Analysis of pRPS3-mCh-tRPS3 RT-qPCR data sets with two experimental repllicates"
...
- pRPS3-mCherry-tRPS3 wild type (WT)

- pRPS3-mCherry-tRPS3 mod0 (mod0)

- pRPS3-mCherry-tRPS3 modA (modA)

- pRPS3-mCherry-tRPS3 modB (modB)

- pRPS3-mCherry-tRPS3 modC (modC)

- pRPS3-mCherry-tRPS3 modD (modD)

- pRPS3-mCherry-tRPS3 modE (modE)
...
strain_levels <- c("pRPS3-mCherry-tRPS3_WT","pRPS3-mCherry-tRPS3_mod0","pRPS3-mCherry-tRPS3_modA","pRPS3-mCherry-tRPS3_modB","pRPS3-mCherry-tRPS3_modC", "pRPS3-mCherry-tRPS3_modD", "pRPS3-mCherry-tRPS3_modE","POT1-ccdB")
...
plate_exp_1 <- read_tsv(here("raw_data_analysis/data/raw_qpcr/JA_20200121-pRPS3-tRPS3mod-n1-ct.txt"),skip=1) %>%
    mutate(well=Pos,cq=Cp) %>%
    left_join(plateplan_exp_1)

plate_exp_2 <- read_tsv(here("raw_data_analysis/data/raw_qpcr/JA_20200204-pRPS3-tRPS3mod-n2-ct.txt"),skip=1) %>%
    mutate(well=Pos,cq=Cp) %>%
...
platesPOT <- plates %>% filter(strain %in% c("pRPS3-mCherry-tRPS3_WT","POT1-ccdB"))
...
platecurve_exp_1 <- read_tsv(
  here("raw_data_analysis/data/raw_qpcr/JA_20200121-pRPS3-tRPS3mod-n1.txt"), 
  skip=2,
...
platecurve_exp_2 <- read_tsv(
  here("raw_data_analysis/data/raw_qpcr/JA_20200204-pRPS3-tRPS3mod-n2.txt"), 
  skip=2,
  col_names=c("well", "SID", "program_no", "segment", "cycle", "time", "temperature", "fluor_raw")) %>%
  debaseline() %>%
  left_join(plateplan_exp_2)
...
ggplot(data=platesamp %>% filter(strain==c("pRPS3-mCherry-tRPS3_WT")),
       aes(x=cycle,y=fluor_signal,linetype=prep_type,group=well, color=tech_rep)) + 
    facet_grid(bio_rep~target_id) + 
    geom_line() +
    scale_linetype_manual(values=c("-RT"="dashed","+RT"="solid")) +
...
ggplot(data=platesmelt %>% filter(strain==c("pRPS3-mCherry-tRPS3_WT")),
       aes(x=temperature,y=dRdT,group=well)) + 
    facet_grid(bio_rep~target_id) + 
    geom_line(aes(color=exp_rep, linetype=prep_type)) +
    scale_linetype_manual(values=c("-RT"="dashed","+RT"="solid")) +
...
platesnorm_pRPS3 <- platesnorm %>% filter(promoter %in% "pRPS3", target_id %in% "mCh-7")

# Save normalised RPS3 constructs
write_csv(platesnorm_pRPS3, here("raw_data_analysis/data/norm_qpcr/pRPS3_tRPS3_n1_n2_norm.csv"))
...
mean_platesnorm_pRPS3mod0 <- platesnorm_pRPS3 %>% 
    ...
    summarize(median_BioRep_pRPS3mod0 = median(delta_cq)) %>%
    ungroup()%>%
    group_by(strain)%>%
...
platesnorm_mod0_pRPS3 <- platesnorm_pRPS3 %>% 
    mutate(delta_delta_cq = delta_cq - mean_platesnorm_pRPS3mod0$mean_pRPS3mod0, remove=FALSE)
```
...
platesnorm_mod0_median <- platesnorm_mod0_pRPS3 %>%
          ...
          # Below is to create columns for strain information using details from the 'Strain' column
                    ...
                    unite(pro_mCh,promoter,mCherry,sep="-", remove=FALSE)%>%
                    unite(ter_mod,terminator,construct,sep="_", remove=FALSE) %>%
                    mutate(tro_mCh = factor(pro_mCh,levels = c("pRPS3-mCherry","pPGK1-mCherry"))) %>% 
