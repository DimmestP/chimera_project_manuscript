---
title: "Analysis of pPGK1-mCh-tTSA1 RT-qPCR data sets with two experimental repllicates"
...
- pPGK1-mCherry-tTSA1 wild type (WT)

- pPGK1-mCherry-tTSA1 mod0 (mod0)

- pPGK1-mCherry-tTSA1 modA (modA)

- pPGK1-mCherry-tTSA1 modB (modB)

- pPGK1-mCherry-tTSA1 modC (modC)

- pPGK1-mCherry-tTSA1 modD (modD)

- pPGK1-mCherry-tTSA1 modE (modE)
...
strain_levels <- c("pPGK1-mCherry-tTSA1_WT","pPGK1-mCherry-tTSA1_mod0","pPGK1-mCherry-tTSA1_modA","pPGK1-mCherry-tTSA1_modB","pPGK1-mCherry-tTSA1_modC", "pPGK1-mCherry-tTSA1_modD", "pPGK1-mCherry-tTSA1_modE","POT1-ccdB")
...
plate_exp_1 <- read_tsv(here("raw_data_analysis/data/raw_qpcr/20200801-pPGK1-tTSA1mod-n1-ct.txt"),skip=1) %>%
    mutate(well=Pos,cq=Cp) %>%
    left_join(plateplan_exp_1)

plate_exp_2 <- read_tsv(here("raw_data_analysis/data/raw_qpcr/20200802-pPGK1-tTSA1mod-n2-ct.txt"),skip=1) %>%
    mutate(well=Pos,cq=Cp) %>%
...
platespPGK1 <- plates %>% filter(strain %in% c("pPGK1-mCherry-tTSA1_WT","pPGK1-mCherry-tTSA1_mod0","pPGK1-mCherry-tTSA1_modA","pPGK1-mCherry-tTSA1_modB",
...
platesPOT <- plates %>% filter(strain %in% c("pPGK1-mCherry-tTSA1_WT","POT1-ccdB"))
...
platecurve_exp_1 <- read_tsv(
  here("raw_data_analysis/data/raw_qpcr/20200801-pPGK1-tTSA1mod-n1.txt"), 
  skip=2,
...
platecurve_exp_2 <- read_tsv(
  here("raw_data_analysis/data/raw_qpcr/20200802-pPGK1-tTSA1mod-n2.txt"), 
  skip=2,
  col_names=c("well", "SID", "program_no", "segment", "cycle", "time", "temperature", "fluor_raw")) %>%
  debaseline() %>%
  left_join(plateplan_exp_2)
...
ggplot(data=platesamp %>% filter(strain==c("pPGK1-mCherry-tTSA1_WT")),
       aes(x=cycle,y=fluor_signal,linetype=prep_type,group=well, color=tech_rep)) + 
    facet_grid(bio_rep~target_id) + 
    geom_line() +
    scale_linetype_manual(values=c("-RT"="dashed","+RT"="solid")) +
    expand_limits(y=0) + 
    labs(title="Amplification curves of pPGK1-mCherry-tTSA1_WT strains (BioRep 1-6) in pPGK1 experiments") + panel_border()
```



### Melt Curves of POT1-ccdB and pPGK1-mCherry-tTSA1_WT strains (all bioreps) against all 4 Targetids (mCh-7, PGK1-ORF, RPS3-ORF, URA3-ORF) (pPGK1)
...
ggplot(data=platesmelt %>% filter(strain==c("pPGK1-mCherry-tTSA1_WT")),
       aes(x=temperature,y=dRdT,group=well)) + 
    facet_grid(bio_rep~target_id) + 
    geom_line(aes(color=exp_rep, linetype=prep_type)) +
    scale_linetype_manual(values=c("-RT"="dashed","+RT"="solid")) +
    scale_x_continuous(breaks=seq(60,100,10),minor_breaks=seq(60,100,5)) + 
    labs(title="Melt curves of pPGK1-mCherry-tTSA1_WT strains (BioRep 1-6) in pPGK1 experiments") + panel_border()
...
platesnorm <- plates  %>% filter(!strain %in% c("POT1-ccdB"), prep_type=="+RT") %>%    
...
Normalisation of delta Cq values against tTSA1-mod0 delta Cq values.   

One **Mean Value.norm of pPGK1-mCherry-tTSA1_mod0** is calculated by taking the median Value.norm among TechReps of each BioRep (of pPGK1-mCherry-tTSA1_mod0), then calculating **one** mean Value.norm among BioReps of pPGK1-mCherry-tTSA1_mod0. 

This value is used to then calculate the delta-delta Cq for all other strains. Delta delta Cq is calculated by subtracting the **Mean Value.norm of pPGK1-mCherry-tTSA1_mod0** from each Value.norm of all other strains. (NOTE - we still have one Value.norm for each TechRep and BioRep here). 
...
write_csv(platesnorm_pPGK1, here("raw_data_analysis/data/norm_qpcr/pPGK1_tTSA1_n1_n2_norm.csv"))
  
# Extracting datapoints for pPGK1-mCherry-tTSA1_mod0 Samples. Calculates the median of each BioRep (collapses TechReps) then the mean Cq  of the mod0 strain (collapses BioReps) (-0.865)
...
platesnorm_mod0_median <- platesnorm_mod0_pPGK1 %>%
          ...
          # Below is to create columns for strain information using details from the 'Strain' column
                    ...
                    unite(pro_mCh,promoter,mCherry,sep="-", remove=FALSE)%>%
                    unite(ter_mod,terminator,construct,sep="_", remove=FALSE) %>%
                    mutate(tro_mCh = factor(pro_mCh,levels = c("pPGK1-mCherry","pPGK1-mCherry"))) %>% 
                    mutate(ter_mod = factor(ter_mod,levels=c("tTSA1_modC","tTSA1_modE","tTSA1_modD","tTSA1_modA",
                                           "tTSA1_modB","tTSA1_mod0","tTSA1_WT")))
...
normalised_plot <- ggplot(data = platesnorm_mod0_median %>% 
    filter(!strain %in% c("pPGK1-mCherry-tTSA1_WT","pPGK1-mCherry-tTSA1_WT")))+
    geom_point(aes(RNA_abundance,ter_mod,colour=ter_mod, shape=bio_rep)) +
    #scale_x_log2nice(name="Fold change in RNA abundance relative to tTSA1-mod0 (log2 scale)",omag = seq(-5,5),scilabels=TRUE) +
    scale_x_log2nice(name="Fold change in RNA abundance relative to tTSA1-mod0 \n (log2 scale)",omag = seq(-5,5),scilabels=FALSE) +
    labs(y="") +
