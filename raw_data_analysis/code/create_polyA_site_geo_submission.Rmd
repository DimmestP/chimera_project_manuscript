...
sample_to_construct_dictionary_QuantSeq_heat_shock <- read_csv(here("raw_data_analysis/data/rna_seq/sample_to_construct_table_QuantSeq_heat_shock.csv"))
...
fill_missing_nucleotide_data_QS_HS <- function(nucleotide_with_known_counts){
  # if first nucleotide has no entry, make a cumulative count 0 entry
 empty_full_plasmid = tibble(read_end = 1:500) %>% 
    filter(!read_end %in% nucleotide_with_known_counts$read_end) %>%
    mutate(terminator = unique(nucleotide_with_known_counts$terminator),
           promoter = unique(nucleotide_with_known_counts$promoter),
           mod = unique(nucleotide_with_known_counts$mod),
...
full_QuantSeq_heat_shock_polyA_reads_from_stop_codon <-  full_QuantSeq_heat_shock_PolyA_reads  %>%
  ...
  filter(read_end > 0, read_end < 500) %>%
  group_by(construct_name,read_end, terminator, promoter, mod,sample, URA3) %>%
  summarise(read_count = n()) %>%
  arrange(construct_name, read_end)
...
empty_full_length_QuantSeq_heat_shock_plasmids <- tibble(construct_name = rep(full_QuantSeq_heat_shock_polyA_reads_from_stop_codon %>% pull(construct_name) %>% unique(), each = 501), 
...
full_length_QuantSeq_heat_shock_plasmids_cumulative_polyA_reads <- full_QuantSeq_heat_shock_polyA_reads_from_stop_codon %>%
  group_by(construct_name, sample, URA3, terminator, promoter, mod) %>%
  do(fill_missing_nucleotide_data_QS_HS(.)) %>%
  dplyr::rename(location = construct_name) |>
  mutate(label = factor(mod),
         promoter = factor(promoter, levels = c("pHSP26","pRPS13","pPGK1", 
                                                "pRPS3", "pTSA1","pTOS6","pPMA1"))) %>%
  inner_join(sample_to_construct_dictionary_QuantSeq_heat_shock, by = c("sample" = "sample_name"))  |>
  mutate(sample = factor(sample, levels = expand.grid("WD_C",7:16) %>% 
                           unite(factor, sep = "") %>% 
                           add_row(factor = c("WD_E2", "WD_E3")) %>% 
                           pull(factor))) |>
  arrange(location, read_end,sample)

full_length_QuantSeq_heat_shock_plasmids_cumulative_polyA_reads_control <-
  full_length_QuantSeq_heat_shock_plasmids_cumulative_polyA_reads |>
  filter(mod == "genomic") %>%
  bind_rows(full_length_QuantSeq_heat_shock_plasmids_cumulative_polyA_reads %>%
              filter(URA3) %>%
              mutate(promoter = "URA3")) |>
  ungroup() |>
  mutate(Gene = str_remove(promoter, "p")) |>
  select(Gene,construct_name,read_count,read_end) |>
  pivot_wider(names_from = construct_name, values_from = read_count) %>%
  arrange(Gene, read_end) %>%
  dplyr::rename(Position_from_stop_codon=read_end)

full_length_QuantSeq_heat_shock_plasmids_cumulative_polyA_reads_plasmid <-
  full_length_QuantSeq_heat_shock_plasmids_cumulative_polyA_reads |>
  filter(mod == "WT", !URA3) |>
  ungroup() |>
  select(construct_name,read_count,read_end) |>
  pivot_wider(names_from = construct_name, values_from = read_count) %>%
  arrange(read_end) %>%
  dplyr::rename(Position_from_stop_codon=read_end)
...
write_tsv(full_length_QuantSeq_heat_shock_plasmids_cumulative_polyA_reads_plasmid, here("raw_data_analysis/data/rna_seq/construct_polyA_bam/QuantSeq_heat_shock/QS_processed_polyA_constructs.tsv"))

write_tsv(full_length_QuantSeq_heat_shock_plasmids_cumulative_polyA_reads_control, here("raw_data_analysis/data/rna_seq/construct_polyA_bam/QuantSeq_heat_shock/QS_processed_polyA_controls.tsv"))
