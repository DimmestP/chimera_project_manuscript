...
Step iteratively adds the motif which reduces the model's AIC the most until the penalty for adding new motifs overcomes the benefit of including a new motif.
Any motifs not included in the model at this point are removed.
This procedure was ran on both @Chan2018 and @Sun2013 decay datasets. 
...
Finally, we selected the specific versions of the HWNCATTWY and TGTAHMNTA motifs by running two addition linear models predicting half life that inferred separate coefficients for each version of its consensus sequence. Coefficients were reported for the significant motif versions (Supplementary Table
\ref{tab:TGTACAATA-motif-coef}, \ref{tab:TTTCATTTC-motif-coef}). We chose instances with similar effect size, statistical significance, and number of occurrences in native transcripts. We chose TTTCATTTC for HWNCATTWY and for TGTAHMNTA chose TGTACAATA over TGTATATTA specifically to avoid the 5nt stretch also found in ATATTC.
...
These positions were selected based on key design criteria; effect on secondary structure (MFE), position of motifs in native 3'UTRs and position of other CREs important for transcriptional control (Supplementary Table \ref{tab:minimum-free-energy-table}, Supplementary Figure \ref{fig:motif-position-histograms}).
...
Similar to the modified RPS3 3'UTR-terminator designs, we chose three 9 nt insertion sites (M1, M2 and M3).
...
Again, median 3'UTRs were extracted from the @Pelechano2013 dataset filtered for the following criteria; 1) contains at least 2 shortlisted motifs of interest, 2) in the top 5% most abundant genes according to @Nadal2019 single cell RNAseq dataset.
...
3) Mutations that resulted in a limited change in the predicted secondary structure and minimum free energy (MFE), see Supplementary Table \@ref(tab:minimum-free-energy-table)) (according to the RNAfold [@Lorenz2011] - http://rna.tbi.univie.ac.at//cgi-bin/RNAWebSuite/RNAfold.cgi). 
...
All analyses made extensive use of the tidyverse [@Wickham2019] and plotting was completed with ggplot2 [@Wickham2016]