...
The pelleted cells were thawed and individually resuspended in 400 $\mu$l RNA binding buffer (Zymo), then transerred to 2 ml screw cap tubes containing zirconia beads (ADD manufacturer), lysed using the Precellys Evolution homogeniser then pelleted by centrifugation at 12,000g for 1.5 minutes. 
The supernatant was transferred to a Zymo Spin IIICG column (Zymo) then centrifuged at 12,000g for 1 minute. 
The flow through was mixed with 1 volume of ethanol then transferred to a Zymo Spin IIC column (Zymo) and centrifuged at 12,000g for 1 minute.
This flow through was discarded and 1 volume of DNA/RNA Prep buffer (Zymo) was added then centrifuged at 12,000g for 1 minute. 
The column was washed with 700 $\mu$l of Zymo DNA/RNA Wash buffer (Zymo) then centrifuged at 12,000g for 1 minute.
The column was washed a second time, but with 400 $\mu$l of Zymo DNA/RNA Wash buffer (Zymo) at 12,000g for 1 minute.
...
2.5 $\mu$l of Random primer mix (NEB) was added to the mixture then separated into 2 PCR tubes (one for -RT and one for +RT).
The RNA is denatured at 70&deg;C then cooled on ice.
Reverse transcription (RT) master mix was prepared, containing 2 $\mu$l of First Strand synthesis buffer, 0.75 $\mu$l of 10mM dNTP mix (NEB), 1.5 $\mu$l of nuclease free water, 0.25 $\mu$l of RNase inhibitor and 0.5 $\mu$l of SuperScript IV Reverse Transcriptase (Invitrogen) per reaction. 
...
For the terminators with short CRE insertions and mutations in the 3'UTR, $\Delta\Delta$Cq values were calculated by normalising mCherry $\Delta$Cq by that of control construct with the same promoter, the mod_NNN strain for tRPS3 and tTSA1, and the WT terminator for tPIR1.
...
Following Cheng et al (2017), we quantified the effect of motifs on transcript half-life using a linear model predicting half life on the basis of codon usage, 3'UTR length, and 3'UTR motif frequency.
...
Explain the greedy algorithm for motif selection, with references.


## Design of modified 3'UTRs for testing the effects of mutated short CREs

3'UTR was selected for screening the effect of removing short CREs of interest individually or in combination to determine their impact on RNA abundance.
To this end, median 3'UTRs were extracted from the Pelechano et al. (2013) dataset filtered for the following critera; 1) contains all 3 short CREs of interest (ATATTC, UGUAMNUA, HWNCAUUWY), 2) < 300 nt in length and 3) in the top 5% most abundant genes according to @Nadal2019 single cell RNAseq data set.
The resulting candidates are summarised in the supplementary table.
The mutation of short CREs for their removal from the 3'UTR were carried out according to the following criteria: 1) at least 50% of the short CRE sequence (specifically the short CRE consensus sequence) was mutated to a base that does not correspond to the consensus sequence.
...
The tRPS3 3'UTR-terminator was modified to incorporate three 9 nt insertion sites for short CREs (M1, M2 and M3).
The M1 was inserted 24 nt downstream of the stop codon, M2 was inserted 15 nt downstream of M1 and the final insert M3 was inserted 4 nt downstream of M2.
These positions were selected based out key design criteria (SUP FIGURE/NOTE X).
A control tRPS3 mod_NNN was designed to incorporate random bases in each insertion site.
Further modified 3’UTR-terminator designs were designed to incorporate short CREs of interest previously identified, within the insertion sites described.
...
The tTSA1 mod_NNN construct contained random bases in the M1, M2 and M3 sites.
Similar to the modified tRPS3 3'UTR-terminator designs, individual short CREs of interest previously described were introduced into the insertion sites, whereby mod_NGG was designed to characterise a candidate stability motif, whilst mod_NTN, mod_NAA, mod_HNH and mod_HTH were designed to characterise candidate decay motifs.

The sequence 'agcgtgCGTCTCgTAGC' was added to the 5'-end of the terminator and the sequence 'CCTCcGAGACGcagcac' was added to the 3'-end of the terminator.
