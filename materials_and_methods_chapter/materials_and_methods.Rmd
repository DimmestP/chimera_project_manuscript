...
We used the median 3'UTR length estimates from the TIF-seq analysis in @Pelechano2013 for the length values and also to assess which sequence to use for motif search. @Pelechano2013 cluster similar TIF-seq reads into major transcript isoforms, mTIFs. The median 3'UTR length for each gene is then calculated over all distinct mTIFs. However, the number of reads mapped to each mTIF can vary from 2 to >10,000. Therefore, we also calculate a weighted median for each gene 3'UTR by calculating the median over the reads mapped to each gene and giving the read the length of the mTIF it has been paired with.  
