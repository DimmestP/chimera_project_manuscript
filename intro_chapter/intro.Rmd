...
Transcription then progresses across until it reaches the terminator, where cleavage and polyadenylation of the nascent RNA transcript is coordinated.
...
However, it is important to distinguish between two concepts of modularity: modular assembly, how easy it is to synthesis constructs consisting of combinations of parts; and composability, how the regulation of the expression of a gene can be decomposed into contributions from individual parts.
...
These experiments cannot measure interactions between different parts and, therefore, implicitly assume that gene expression is composed of independent contributions of parts.

Computational methods for the discovery and quantification of short CREs, such as sequence motifs recognised by regulatory proteins, also tend to rely on an independent model of composability.
...
However, framing the search for composability around independent contributions from the shortest possible sequences overlooks multi-part motifs, interactions between motifs, and the fact that motifs may be active only in specific contexts.

The importance of the interactions between CRE alone has been clear since the discovery of cooperative DNA binding by transcription factors [@Giniger1988].
...
However, 5% of the constructs had, on average, a 13-fold deviation [@Kosuri2013].
Another study of promoter-terminator swaps in budding yeast revealed 0.5 relative fold changes due to promoter-terminator interactions [@Ito2013], even though these CREs are separated by 700nt of coding sequence. 

Here, we provide further evidence for the importance of interactions between regulatory regions, and show the limitations of decomposing gene expression into discrete contributions from short CREs. 
