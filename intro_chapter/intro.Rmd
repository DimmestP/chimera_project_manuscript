...
Modular assembly is now routine: modern DNA synthesis and cloning techniques allow the construction of complex genetic pathways through the combinatorial assembly of chosen parts [@Curran2013; @Lee2015; @Garcia2018; @Andreou2018]. 
...
However, framing the search for CREs around independent contributions from short motifs overlooks multi-part motifs (**citation**), interactions between motifs, and the fact that motifs may be active only in specific contexts.
...
Through an extensive pair-wise assessment of interaction dynamics for 120 promoter-terminator combinations asseembled by modular cloning, we highlight the limitations of decomposing gene expression regulation into promoter (including 5’UTR), coding sequence, and terminator (including 3’UTR) modules. 
We show that the quantitative effect of terminators on protein production depends upon both promoter and coding sequence. We then investigate the effect of promoter and terminator context on the regulatory behaviour of short CREs in 3’UTRs. 
In order to select putative short CREs, we adapt a method to computationally predict mRNA log2-half-life as a linear combination of codon usage and motif counts [@Cheng2017]. 
We then quantify the effect on mRNA and protein levels of selected short CREs when inserted in, or removed from, 3’UTRs in diverse reporter gene constructs. 
