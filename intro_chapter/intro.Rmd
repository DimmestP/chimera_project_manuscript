...
Abstracting specific characteristics to individual elements of a gene has facilitated the assumption that parts are modular or interchangeable with other similar parts.
It is tempting to assume that characterisation of a part in a single context (of other parts) is sufficient to predict its function in a different context.
This assumption blurs two concepts of modularity: modular assembly, how easy it is to construct combinations of parts; and composability, how function and regulation can be decomposed into contributions from individual parts.
Assembly of parts that are non-composable would result in assembled genes with unpredictable function or quantitative regulation.
...
Because these experiments do not measure interactions between different parts, they rely on the untested assumption that those parts function more or less identically across different contextx, that is, that modularly assembled genes are also composable.
...
The importance of the interactions among CREs alone has been clear since the discovery of cooperative DNA binding by transcription factors [@Ptashne1980].
...
Our results demonstrate that the incorporation of candidate decay motifs indeed results in a decrease in mRNA abundance, however their quantitative contribution depends on both the local 3'UTR context and the more distant promoter context.
