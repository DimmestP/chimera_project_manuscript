...
For these constructs we performed poly(A)-proximal RNA-seq using the QuantSeq Rev protocol with paired-end reads (REF and see methods).
...
We detected 1000s (check) of reads on each reporter construct, which is enough to quantify expression confidently as well as to assign poly(A) sites.
We checked that counts of all other RNAs are highly correlated between samples, giving us confidence that changes in construct detection are meaningful (NEEDS SUPPLEMENTARY FIGURE PAIRS PLOT OR CORRELATION PLOT).
...
We checked that in other genes the poly(A) site locations were highly reproducible across samples, giving us confidence that changes in construct poly(A) sites are meaningful (SUPP FIGURE for some of URA3, RPS3, TSA1, PGK1).
Poly(A) sites are / are not (delete as appropriate) in the same relative positions in native loci and the construct with wild-type terminator (SUPP FIGURE).
...
Surprisingly, tRPS3 mod_NNN constructs appear to be using a novel upstream poly(A) site, that appears in about $50%$ percent of reads and is located upstream of the 3rd motif insertion site (Figure \@ref(fig:quantseq-polyA-site-usage)B).
...
```{r quantseq-polyA-site-usage, fig.cap="\\textbf{Inserting motifs into RPS3 and TSA3 terminators changes 3'UTR length.} A subset of constructs were chosen to investigate changes in polyA site usage; mod\\_NAA, mod\\_HTH, mod\\_NTN, WT and mod\\_NNN within three promoter-terminator contexts; pRPS3-tRPS3, pPGK1-tRPS3 and pTSA1-tTSA1. (\\textbf{A}) Comparison of construct transcript abundance as independently measured by qPCR and RNA-Seq assays. Transcript abundance was normalised to the median abundance of  plasmid URA3 and genomic PGK1, RPS3 and/or TSA1 transcripts for each construct. Fold change is relative to the mod\\_NNN construct in each promoter-terminator context. (\\textbf{B}) Cumulative count of reads mapped downstream of WT (grey) and mod\\_NNN (black) construct stop codons as fraction of total reads mapped to the constructs terminator. (\\textbf{Left}) shows the polyA sites for the pRPS3-tRPS3 and pPGK1-tRPS3 promoter-terminator constructs. (\\textbf{Right}) shows the same for the pTSA1-tTSA1 constructs. WT reads have been shifted downstream to align with the mod\\_NNN sequence by accounting for motif insertion sites. Suspected polyA sites have been highlighted by a black vertical line. (\\textbf{C}) Similar to Figure \\textbf{B} but with each motif insertion construct plotted separately. Columns designate cumulative plots from different terminator constructs and rows designate cumulative plots from different promoter-terminator contexts.", fig.align='center',out.width="85%"}
knitr::include_graphics(here("results_chapter/figures/polya_usage_plot_QuantSeq.png"))
```

We next asked if the alternate poly(A) sites might have different decay rates, using 5Pseq to measure decapped transcripts, which are decay intermediates (ref to 5Pseq paper).
Our 5Pseq data confirms the QuantSeq results and provides no evidence for differential decay of isoforms (Sup Fig \@ref(fig:polyA-site-usage-5Pseq)).
For control genes and for the constructs with wildtype tRPS3, 5Pseq identifies indistinguishable poly(A) sites from QuantSeq (CHECK WITH NEW SUPP FIGURE, and Sup Fig \@ref(fig:polyA-site-usage-5Pseq)B).
For a subset of tRPS3 constructs with additional poly(A) sites, that we were able to detect confidently, 5Pseq shows poly(A)-site profiles that are indistinguishable from QuantSeq (Sup Fig \@ref(fig:polyA-site-usage-5Pseq)B).
The lack of detectable differential decay, and the fact that even the upstream poly(A) site lacks only one inserted motif in the constructs, argues that the major effects of motif insertions on mRNA decay are unlikely to occur due to alternative poly(A) site formation.
Although further work would be needed to verify this hypothesis in detail, our results are consistent with the motifs acting by recruiting RNA-binding proteins to polyadenylated transcripts.
