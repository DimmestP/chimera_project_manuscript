...
We selected promoters and terminators from native yeast genes spanning a variety of different expression patterns summarised in \ref{tab:terminator-summary-table}.
...
readr::read_csv(here("results_chapter/data/terminator_summary_table.csv")) %>%
  knitr::kable(format = "latex", align = "c", caption = "\\textbf{Summary of full terminator library.} The common gene name from which the terminator is extracted from is included alongside its systematic name. The median 3'UTR reported by \\cite{Pelechano2013} is a median over the lengths of each distinct isoform they detect. Our weighted median is weighted by the number of reads mapped to each isoform. The inferred terminator length used in this paper includes up to 100bp downstream of the median Pelechano 3'UTR length. The usage of each motif is signified by S, for promoter-terminator swaps, or M, for motif insertion or deletion. A short summary of the native genes ontology is included.") %>%
  kableExtra::column_spec(2:5, width = "1.7cm") %>%
...
Approximately 1.7% of the variance is explained by these 7 motifs, with 42.0% explained by codon usage see Sup Table \@ref(tab:hlife-variance-table), consistent with previous analyses [@Presnyak2015;@Cheng2017].
...
readr::read_csv(here("results_chapter/data/motif_summary_table.csv")) %>%
  knitr::kable(format = "latex", align = "c", caption = "\\textbf{Summary of shortlisted motif charactoristics.} The first three columns hold the consensus sequences for each motif and, if appropriate, the exact versions removed from or inserted into host terminators. Each motif's coefficent given by the linear model predicting either \\cite{Chan2018} or \\cite{Sun2013} half life data sets are included. Finally, the table includes any suggested motif functions.") %>%
