...
To investigate the context dependence of regulatory regions, we created a library of 120 constructs containing all combindations of 10 terminators, 6 promoters and 2 ORFs.
t and p prefixes are added to native gene names to denote references to terminator/promoter regions.
The ORF either encoded a mCherry or mTurq fluorescent protein and a platereader assay measured each construct's fluorescence.
Promoter choice dominated overall gene expression with over 100 fold changes in fluorescence between the four highest expressing promoters, \@ref(fig:pro-ter-platereader-mCherry-mTurq-raw)A, and the two lowest expressing promoters, \@ref(fig:pro-ter-platereader-mCherry-mTurq-raw)B.
...
We also measured the mRNA levels of a subset of mCherry constructs and confirmed the effects seen on protein expression correlate with transcript levels, \@ref(fig:pro-ter-platereader-mCherry-mTurq-raw)C.

We normalised the fluorescence of each construct to the PGK1 terminator its promoter-ORF set in order to explore the context dependence of each terminator by comparing relative contributions to gene expression, \@ref(fig:pro-ter-platereader-mCherry-mTurq-norm)A.
...
For both native genes, selecting a terminator length longer than the suspected 3'UTR enables greater mRNA expression, \@ref(fig:pro-ter-platereader-mCherry-mTurq-norm)B. 
