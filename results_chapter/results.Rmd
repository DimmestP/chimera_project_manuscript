...
We selected promoters and terminators from native yeast genes spanning a variety of different expression patterns and functions, summarised in Table \ref{tab:terminator-summary-table}.
...
We also confirmed that most differences in protein outputs are already present in mRNA abundance by checking a subset of mCherry constructs using RT-qPCR, Fig \@ref(fig:pro-ter-platereader-mCherry-mTurq-raw)C.

Terminators also affect protein output, seen by 5-fold changes in fluorescence within the same promoter-CDS sets, relative to the tPGK1 terminator of each group, in Fig \@ref(fig:pro-ter-platereader-mCherry-mTurq-norm)A.
...
Note that we used the longer 543bp SRO9 terminator, and a similarly extended 421bp tPMA1, for the constructs in Fig \@ref(fig:pro-ter-platereader-mCherry-mTurq-raw).
...
Approximately 1.7% of the variance is explained by these 7 motifs, with 42.0% explained by codon usage see Supp. Table \@ref(tab:hlife-variance-table), consistent with previous analyses [@Presnyak2015;@Cheng2017].
The top 7 most significant motifs from the @Chan2018 data showed similar regulatory behaviour when tested on their own in the @Sun2013 data, except for TGTAAATA which swapped from being stabilising to destabilising, Fig \@ref(fig:hlife-decay-model)B.
...
Protein fluorescence results show a correlation between mRNA expression and protein expression (Sup Fig \@ref(fig:protein-vs-RNA-plot-motifs)).
...
We find that the removal of almost any decay motif from tPIR1 results in an increase in mRNA levels (Fig \@ref(fig:tPIR1-design-and-qpcr)B)).
...
We checked if differences in mRNA abundance were reflected in protein expression, by comparing the RT-qPCR with fluorescence plate reader measurements for a a subset of constructs with native promoter-terminator pairings (Fig \@ref(fig:protein-vs-RNA-plot-motifs)). 
